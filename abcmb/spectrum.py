...
#   - flat (curvature=False, the common case incl. OLE runs): the fast
#     tabulated spherical-Bessel tables below + a sparse-ell CubicSpline, the
#     long-validated origin/main path.
...
class SpectrumSolver(eqx.Module):
    ...
    def get_Cl(self, PT, BG, params):
        ...
        if self.curvature:
            ...
            # Flat geometry (the common path, incl. OLE runs): fast sparse-ell
            # tabulated-Bessel transfer + CubicSpline. Scalar Cls are smooth, so
            # the spline meets the 1% scalar accuracy bar (origin/main's long-
            # validated path) and avoids the every-ell recurrence walk that the
            # curved branch needs.
