...
The dual-path change (this session) gates the scalar get_Cl on the static
SpectrumSolver.curvature flag:
...
This driver does the decisive cross-check that the split is self-consistent:
at omega_k = 0 the curved recurrence reduces to the flat j_l, so the two
branches must agree to the (sparse-ell CubicSpline) scalar floor. It also runs
a genuinely-curved config to confirm the curved branch still produces finite,
sensible Cls after the get_Cl restructure (curved logic is unchanged, just
re-indented under `if self.curvature`).

Cheap: 3 forward calls (1 compile + 1 warm each), tensors=False, no AD.
...
def run(curvature, omega_k, label):
    m = Model(l_max=2500, lensing=False, tensors=False,
              curvature=curvature, omega_k_ref=omega_k)
...
# Cross-check: flat table path vs curved recurrence at K=0 must agree to the
# scalar (sparse-ell CubicSpline) floor. Compare on the shared ell grid, ell>=2.
def relerr(a, b):
    m = np.abs(b) > 0
